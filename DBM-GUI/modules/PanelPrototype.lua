...
function PanelPrototype:CreateEditBox(text, value, width, height)
	local textbox = CreateFrame("EditBox", "DBM_GUI_Option_" .. self:GetNewID(), self.frame, "DBM_GUI_FrameEditBoxTemplate")
...
do
	...
	function PanelPrototype:CreateCheckButton(name, autoplace, textLeft, dbmvar, dbtvar, mod, modvar, globalvar, isTimer)
		...
		if dbtvar then
			button:SetScript("OnShow", function(self)
				self:SetChecked(DBM.Options[dbtvar])
			end)
			button:SetScript("OnClick", function()
				DBT:SetOption(dbtvar, not DBM.Options[dbtvar])
			end)
		end
